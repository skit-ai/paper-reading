...
developing modelling. They also do a lot of multitask learning and snorkelish
weak supervision, including the recent slicing abstractions for fine grained
quality control.
...
specific representations which act as computationally cheaper way of replicating
a Mixture of Experts style models.

Like with labelling functions, we have the slice membership predicted using
heuristics which are cheap and noisy. This membership value along with slice
representations (and slice prediction confidences) help creating the slice aware
representation to be used for the final task. The appendix has few good examples
for slicing functions.
